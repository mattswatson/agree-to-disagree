...
def calculate_all_shap_importance(model, dataset, device, baseline='random', multiclass=False, flatten=False,
    ...
    with tqdm(total=len(dataset)) as progress:
        for inputs, targets in dataloader:
            ...
            #target_label = np.where(targets[0].cpu().numpy() == 1)
            #print('target label')
            if target != targets[0]:
                continue
            ...
            if not multiclass:
                ...
                shap_vals = shap.attribute(inputs, baselines=baseline, target=target)
...
def train_svm(shap_vals_neg, shap_vals_pos, feature_names=None, test_split=0.3, kernel=None, save=None,
              save_results=None, results_name=''):
